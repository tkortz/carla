...
class ClientSideBoundingBoxes(object):
    ...
    def save_bounding_boxes(start_frame_id, frame_id, bboxes_and_target_info, display_dim, camera_pose, target_str):
        ...
        try:
            f = open("{0}/{1}_bboxes_{2}.txt".format(RESULTS_DIR, target_str, start_frame_id), 'a')
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image, sensor, shouldDisplay):
        ...
        # if self.recording:
        # print(self.sensors[0].get_transform().rotation.pitch, self.sensors[0].get_transform().rotation.roll)
        if self.display_bboxes and self.save_images:
            image.save_to_disk('%s/%s/%08d' % (RESULTS_DIR, shortName, image.frame))
